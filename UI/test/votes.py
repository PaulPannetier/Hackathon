...
def resetElections(candidates, voters):
    ...
    Réinitialise les scores à 0 et les attributs hasVoted à False"""
...
def copeland(candidates:dict, voters:dict):
    """Dict[int,Candidate] x Dict[int,Voter] -> int
    Méthode de Copeland
    Args:
    ...
    for id in candidates:
        if id_vainqueur==0:
            id_vainqueur=id
        elif candidates[id].score > candidates[id_vainqueur].score:
            id_vainqueur = id
    return id_vainqueur
...
def simpson(candidates:dict,voters:dict):
    ...
    for id in candidates:
        if id_vainqueur==0:
            id_vainqueur=id
        elif candidates[id1].score > candidates[id_vainqueur].score:
            id_vainqueur = id
    return id_vainqueur
