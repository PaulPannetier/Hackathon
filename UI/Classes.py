...
class Drawing(QWidget):
    ...
    def coord_to_pos(self,x=None,y=None):
        """Convertit les coordonnées de la souris en position sur l'axe des abscisses ou des ordonnées

        Args:
            x (float, optional): Defaults to None.
            y (float, optional): Defaults to None.

        Returns:
            coordonnées converties, defaults to x = y si x ou y vaut None
        """
        if x==None:
            return 400-y
        if y==None:
    ...
    def drawEllipse(self,x,y,color,size):
        """dessine un cercle de centre (x,y) de couleur color et de rayon size

        Args:
            x (float)
            y (float)
            color (QColor): couleur du cercle
            size (int): rayon du cercle en pixels
        """
